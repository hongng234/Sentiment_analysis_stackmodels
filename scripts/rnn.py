...
from keras.layers import Dense, Embedding, Input, CuDNNGRU, GRU, LSTM, Bidirectional, CuDNNLSTM, GlobalMaxPool1D, GlobalAveragePooling1D, Dropout, Lambda, Concatenate, TimeDistributed
...
def LSTMKeras(embeddingMatrix=None, embed_size=400, max_features=20000, maxlen=100):
    inp = Input(shape=(maxlen, ))
    x = Embedding(input_dim=max_features, output_dim=embed_size, weight=[embeddingMatrix])(inp)
...
def HRNN(embeddingMatrix=None, embed_size=400, max_features=20000, max_nb_sent=3, max_sent_len=40, trainable=True, use_additive_emb=False):
    ...
    embed = Embedding(input_dim=max_features, output_dim=embed_size, weights=[embeddingMatrix], trainable=trainable)(sent_inp)
